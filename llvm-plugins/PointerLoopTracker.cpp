...
static cl::opt<std::string>
    FSGOFile("Lfsgout", cl::desc("File to write FreeSentry call analysis too"),
             cl::init("/tmp/fs-callmodel.raw"), cl::Optional);
static cl::opt<std::string>
    FSGIFile("Lfsgin", cl::desc("File to read FreeSentry call model from"),
...
class FreeSentryShared {
  public:
    ...
    bool callsfree(String funcname) {
        ...
        assuming free is called" << "\n"); return true; } else { bool freecalled
        = freecalls[funcname];
              // debug
              if (freecalled)
                                        ...
                                        if (dominates && EXBB) {
                                            ...
                                            Value *newcast =
                                                B.CreatePointerCast(
                                                    ...
                                                    DL->getIntPtrType(
                                                        M->getContext(), 0));
                                            Value *load_cast = B.CreateIntToPtr(
                                                newcast,
                                                B.getInt64Ty()->getPointerTo());
                                            Value *LI = B.CreateLoad(load_cast);

                                            // Args.push_back
                                            // (B.CreatePtrToInt(newcast,
                                            // DL->getIntPtrType(M->getContext(),
                                            // 0)));
                                            // Args.push_back(B.CreatePtrToInt(LI,
                                            // DL->getIntPtrType(M->getContext(),
                                            // 0))); Value *arg1 =
                                            // CI->getOperand(0); Value *arg2 =
                                            // CI->getOperand(1);
                                            ...
                                            Args.push_back(B.CreatePtrToInt(
                                                LI, DL->getIntPtrType(
                                                        M->getContext(), 0)));
                                            /* TODO: remove this
                                             * Below call is commented to remove
...
static RegisterPass<FreeSentryLoop> Y("FreeSentryLoop",
                                      "UAF Protection Loop optimization");
/*
...
static void registerFreeSentryLoop(const PassManagerBuilder &,
    ...
    DEBUG_MSG(errs() << "Adding FreeSentryLoop\n");
    PM.add(new FreeSentryLoop());
}
static RegisterStandardPasses
    RegisterFreeSentryLoop(PassManagerBuilder::EP_OptimizerLast,
                           registerFreeSentryLoop);
static RegisterStandardPasses
    RegisterFreeSentryLoop0(PassManagerBuilder::EP_EnabledOnOptLevel0,
                            registerFreeSentryLoop);

...
STATISTIC(FreeSentryRegptr, "Counts number of register pointer calls that were "
                            "added before optimization");
STATISTIC(
    FreeSentryRegptrCall,
    "Counts number of register pointer calls that were added due to calls");
STATISTIC(
    FreeSentryRegptrStore,
    "Counts number of register pointer calls that were added due to stores");
STATISTIC(FreeSentryLoopStat, "Counts number of register pointer calls that "
                              "were moved due to loop optimization");

static cl::opt<std::string>
    FSGOFile("fsgout", cl::desc("File to write FreeSentry call analysis too"),
             cl::init("/tmp/fs-callmodel.raw"), cl::Optional);
static cl::opt<std::string>
    FSGIFile("fsgin", cl::desc("File to read FreeSentry call model from"),
  ...
  public:
    ...
    bool callsfree(String funcname) {
        if (freecalls.find(funcname) == freecalls.end()) {
            DEBUG_MSG(errs() << "FSL: can't find function in freecalls, "
                                "assuming free is called"
                             << "\n");
            return true;
        } else {
    ...
    inline Value *getBoundedAddr(Value *obj_addr) {
        if (GetElementPtrInst *GEP = dyn_cast<GetElementPtrInst>(obj_addr)) {
            if (1 == GEP->getNumIndices()) {
                ...
                if (Instruction *SubI =
                        dyn_cast<Instruction>(GEP->getOperand(1))) {
                    if (SubI->getOpcode() == Instruction::Sub) {
                        ConstantInt *C =
                            dyn_cast<ConstantInt>(SubI->getOperand(0));
                        if (C && C->isZero())
                            return GEP->getOperand(0);
                    }
                }
...
struct FreeSentry : public FunctionPass, FreeSentryShared {
    ...
    FreeSentry(bool setflag) : FunctionPass(ID) {
        flag = setflag;
        FreeSentry();
    }
    ...
    bool doInitialization(Module *Mod) {
        ...
        M->getOrInsertFunction(UAFFUNC, VoidTy, IntPtrTy, IntPtrTy);
        ...
        for (inst_iterator i = inst_begin(F), e = inst_end(F); i != e; ++i) {
            ...
            if (isa<CallInst>(I)) {
                ...
                if (CalleeType->isPointerTy()) {
                    DEBUG_MSG(errs()
                              << "FS: called function returns a pointer\n");
                    prevcall = true;
                }
                ...
                if (prevcall) {
                    ...
                    if (!freecalled) {
                        if (!HAT(BCI, NULL)) {
                            DEBUG_MSG(errs() << "FS: No address taken of value "
                                                "and no calls to free in "
                                                "function, no need to register "
                    ...
                    M->getOrInsertFunction(
                        UAFFUNC, Type::getVoidTy(M->getContext()),
                    ...
                    DEBUG_MSG(errs() << "FS (call): adding registerptr for"
                                     << *I << "\n");
                    ...
                    if (BC_operand->getType()->getTypeID() ==
                        Type::DoubleTyID) {
                        continue;
                    }
                    ...
                    Value *cast = B.CreatePointerCast(
                        GEP, DL->getIntPtrType(M->getContext(), 0));

                    /* Metaalloc: Need to read value of the pointer operand */
                    Value *load_cast =
                        B.CreateIntToPtr(cast, B.getInt64Ty()->getPointerTo());
                    DEBUG_MSG(errs() << "FS (call): Load pointer: "
                                     << *load_cast << "\n");
                    ...
                    Args.push_back(B.CreatePtrToInt(
                        cast, DL->getIntPtrType(M->getContext(), 0)));
                    Args.push_back(B.CreatePtrToInt(
                        LI, DL->getIntPtrType(M->getContext(), 0)));
                ...
                DEBUG_MSG(errs() << "FS: Pointer operand: "
                ...
                DEBUG_MSG(errs()
                          ...
                          << ")\n");
                DEBUG_MSG(errs() << "FS: Value operand: "
                                 << *SI->getValueOperand() << "\n");
                DEBUG_MSG(errs()
                          << "FS: Value type: "
                ...
                if (SI->getValueOperand()->getType()->isPointerTy()) {
                    if (isa<GetElementPtrInst>(valop)) {
                        ...
                        GetElementPtrInst *GI =
                            dyn_cast<GetElementPtrInst>(valop);
                        ...
                        if (isa<LoadInst>(GI->getPointerOperand())) {
                            ...
                            if (SI->getPointerOperand() ==
                                ...
                                DEBUG_MSG(
                                    errs()
                                    << "FS: Pointer loaded, added to and then "
                                       "stored again, ignore\n");
                                continue;
                            }
                        }
                    ...
                    if (!freecalled) {
                        if (isa<Instruction>(valop)) {
                            ...
                            if (!HAT(AI, NULL)) {
                                DEBUG_MSG(
                                    errs()
                                    << "FS: No address taken of value and no "
                                       "calls to free in function, no need to "
                                       "register this particular pointer\n");
                                continue;
                            }
                        }
                    }

                    // if (strstr(F.getName().str().c_str(), "Perl_ck_split") !=
                    // NULL) {
                    //    continue;
                    //}
                    // Module *M = F.getParent ();
                    ...
                    std::vector<Value *> Args;
                    // Value *cast =
                    // B.CreatePointerCast (SI->getPointerOperand (),
                    // DL->getIntPtrType(M->getContext(), 0));

                    // Value *LI = B.CreateLoad(cast);
                    // Value *LI = B.CreateLoad(SI->getPointerOperand());
                    DEBUG_MSG(errs() << "FS: adding registerptr for"
                                     << *SI->getPointerOperand() << "\n");

                    Args.push_back(
                        B.CreatePtrToInt(SI->getPointerOperand(), IntPtrTy));
...
struct FreeSentryLoop : public LoopPass, FreeSentryShared {
    ...
    FreeSentryLoop(bool setflag) : LoopPass(ID) {
        flag = setflag;
        FreeSentryLoop();
    }
            ...
            if (LI->getLoopFor(BB) == L) {
                for (BasicBlock::iterator I = BB->begin(), E = BB->end();
                    ...
                    if (isa<CallInst>(inst)) {
                        DEBUG_MSG(errs() << "FSL: Found call instruction: "
                                         << *inst << "\n");
                        CallInst *CI = dyn_cast<CallInst>(inst);
                        Function *F = CI->getCalledFunction();
                        if (F) {
                            StringRef funcname = F->getName();
                            DEBUG_MSG(errs() << "FSL: Function called: "
                                             << funcname << "\n");
                            if (funcname == UAFFUNC) {
                                *lcallsregptr = true;
                    ...
                    if (isa<CallInst>(inst)) {
                        ...
                        if (F) {
                            ...
                            DEBUG_MSG(errs() << "FSL: Function called: "
                                             << funcname << "\n");
                            if (funcname == UAFFUNC) {
                                if (CastInst *cast = dyn_cast<CastInst>(
                                        CI->getArgOperand(0))) {
                                    Value *arg = cast->getOperand(0);
                                    DEBUG_MSG(errs()
                                              << "FSL: Arg: " << *arg << "\n");
                                    if (Instruction *AI =
                                            dyn_cast<Instruction>(arg)) {
                                        if (HAT(AI, L)) {
                                            DEBUG_MSG(errs()
                                                      << "FSL: Address taken: "
                                                      << "\n");
                                        ...
                                        if (GetElementPtrInst *GEP =
                                            ...
                                            DEBUG_MSG(errs()
                                                      << "FSL: GEP instruction:"
                                                      << GEP << "\n");
                                            GEP = NULL; // TODO: This is just to
                                                        // make compiler happy
                                            continue;
                                        }
                                        DEBUG_MSG(
                                            errs()
                                            << "FSL: checking domination\n");
                                        // no address taken and not a GEP
                                        // instruction move outside of loop
                                        ...
                                        if (dominates && EXBB) {
                                            ...
                                            M->getOrInsertFunction(
                                                ...
                                                DL->getIntPtrType(
                                                    M->getContext(), 0));
                                            Constant *regptr_def =
                                                M->getFunction(UAFFUNC);

                                            Function *regptr =
                                                dyn_cast<Function>(regptr_def);
                                            regptr->setCallingConv(
                                                CallingConv::C);

                                            Instruction *
                                                insertBeforeInstruction = EXBBI;
                                            IRBuilder<> B(
...
struct FSGraph : public FunctionPass {
    ...
    FSGraph(bool setflag) : FunctionPass(ID) {
        flag = setflag;
        FSGraph();
    }
    ...
    bool doInitialization(Module &M) {
        ...
        std::error_code EC;
        outfile = new raw_fd_ostream(filename.c_str(), EC, sys::fs::F_Append);
        return false;
    }
    ...
    void dumpFunction(Function *F) {
        ...
        FSSet *fcalls = getOrInsertFunction(F);
        *outfile << F->getName() << ": ";
        for (FSSet::iterator I = fcalls->begin(), IE = fcalls->end(); I != IE;
             ++I) {
            String func = *I;
            *outfile << func << " ";
        }
...
// static RegisterPass <FreeSentryLoop> Y("FreeSentryLoop", "UAF Protection Loop
// optimization");
/*
INITIALIZE_PASS_BEGIN(FreeSentryLoop, "FreeSentryLoop", "UAF Protection Loop
...
INITIALIZE_PASS_END(FreeSentryLoop, "FreeSentryLoop", "UAF Protection Loop
optimization", false, false)
...
static void registerFreeSentry(const PassManagerBuilder &,
    ...
    DEBUG_MSG(errs() << "Adding FreeSentry\n");
    PM.add(new FreeSentry());
}
static RegisterStandardPasses
    RegisterFreeSentry(PassManagerBuilder::EP_OptimizerLast,
                       registerFreeSentry);
static RegisterStandardPasses
    RegisterFreeSentry0(PassManagerBuilder::EP_EnabledOnOptLevel0,
                        registerFreeSentry);

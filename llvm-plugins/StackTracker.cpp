...
cl::opt<bool> MergedStack(
    "mergedstack",
    cl::desc("Merge the static stack allocas before generating metadata"),
    cl::init(false));
...
struct StackTracker : public FunctionPass {
    ...
    void MergeAllocas(std::vector<AllocaInst *> &oldStaticAllocas,
        ...
        // Compute the total size of static objects (including desired
        // alignment). Traverse in reverse alloca order.
        unsigned long staticOffset = 0; // Current stack top.
        std::vector<unsigned long> offsetVector;
        const std::vector<llvm::AllocaInst *> &oldStaticAllocasConst =
            oldStaticAllocas;
        for (auto &AI : oldStaticAllocasConst) {
            ...
            uint64_t size =
                DL->getTypeAllocSize(Ty) * arraySize->getZExtValue();
            if (size == 0)
                size = 1; // Don't create zero-sized stack objects.
            staticOffset += size;
        }
        AllocaInst *newAI = AllocaBuilder.CreateAlloca(
            Int8Ty, ConstantInt::get(IntPtrTy, staticOffset));
        ...
        int offsetPos = 0;
        for (auto &AI : oldStaticAllocasConst) {
            Value *newObject = AllocaBuilder.CreateGEP(
                newAI, ConstantInt::get(IntPtrTy, offsetVector[offsetPos]));
            AI->replaceAllUsesWith(
                AllocaBuilder.CreatePointerCast(newObject, AI->getType()));
            ++offsetPos;
        }
    ...
    Instruction *GetInitPosition(AllocaInst *AI, Instruction *firstNonAlloca,
                                 LoopInfo *LI, DominatorTree *DT) {
        std::set<std::pair<const Instruction *, const Value *>> SideEffects;
        SM->AccumulateUnsafeSideEffects(
            AI, SM->GetStaticAllocaAllocationSize(AI), SideEffects);
        const BasicBlock *dominator = (SideEffects.begin()->first)->getParent();
        for (auto &pair : SideEffects) {
            dominator = DT->findNearestCommonDominator(pair.first->getParent(),
                                                       dominator);
        }
        while (LI->getLoopFor(dominator)) {
            dominator = DT->getNode(const_cast<BasicBlock *>(dominator))
                            ->getIDom()
                            ->getBlock();
        }
        if (dominator != firstNonAlloca->getParent()) {
            return const_cast<Instruction *>(
                &(*dominator->getFirstInsertionPt()));
        } else {
    ...
    virtual bool runOnFunction(Function &F) override {
        ...
        for (auto &i : F.getEntryBlock()) {
            ...
            if (SM->IsSafeStackAlloca(AI, Size)) {
                AllocaInst *newAI = AllocaBuilder.CreateAlloca(
                    AI->getAllocatedType(), AI->getArraySize(), AI->getName());
        ...
        if (!MergedStack) {
            // Just create new allocas at the top of the function if no merging
            // requested
            for (auto &AI : oldStaticAllocas) {
                AllocaInst *newAI = AllocaBuilder.CreateAlloca(
                    AI->getAllocatedType(), AI->getArraySize(), AI->getName());
            ...
            // Merge the static allocas into a single alloca for cheaper
            // metadata management
        ...
        for (auto AI : interestingAllocas) {
            // unsigned long Size = SM->GetStaticAllocaAllocationSize(AI);
            // Create metadata if needed and insert it before the regular
            // allocation Performed here not to disturb the IRBuilder metaData
            // is scoped here to be accessible during initialization
            Value *metaData;
            if (DeepMetadata) {
                metaData = new AllocaInst(MetaDataTy, 0, nullptr,
                                          "MetaData_" + AI->getName(), AI);
            }
            ...
            if (staticAllocas.count(AI) == 0) {
                ...
                insertBeforeInstruction =
                    GetInitPosition(AI, firstNonAlloca, LI, DT);
            }
            ...
            */
            ...
            if (DeepMetadata) {
                ...
                for (unsigned long i = 0;
                    ...
                    B.CreateStore(ConstantInt::get(IntPtrTy, 0, 0), metaField,
                                  true);
                }
            }
        }
    ...
    bool doInitialization(Module &Mod) override {
        ...
        if (!FixedCompression) {
            ...
            std::string functionName =
                "metaset_alignment_" + std::to_string(MetadataBytes);
            ...
            // declare i64 @metaset_alignment(i64, i64, iM, i64)
            std::string functionName3 =
                "metaset_fast_" + std::to_string(MetadataBytes);
            M->getOrInsertFunction(functionName3, IntPtrTy, IntPtrTy, IntPtrTy,
                                   IntMetaTy, IntPtrTy, IntPtrTy, IntPtrTy);
...
static RegisterPass<StackTracker> X("stacktracker", "Stack Tracker Pass", true,
                                    false);

static void registerStackTracker(const PassManagerBuilder &,
                                 legacy::PassManagerBase &PM) {
    DEBUG_MSG(errs() << "Adding StackTracker\n");
    PM.add(new StackTracker());
}
static RegisterStandardPasses
    RegisterStackTracker(PassManagerBuilder::EP_OptimizerLast,
                         registerStackTracker);
static RegisterStandardPasses
    RegisterStackTracker0(PassManagerBuilder::EP_EnabledOnOptLevel0,
                          registerStackTracker);
